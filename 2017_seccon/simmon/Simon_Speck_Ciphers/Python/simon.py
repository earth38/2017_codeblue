...
if __name__ == "__main__":
    #w = SimonCipher(0x534543434f4e7b787878787d, key_size=96, block_size=64)
    #w = SimonCipher(0x1918111009080100, key_size=64, block_size=32)
    #t = w.encrypt(0x65656877)
    #exit()
    #t = w.decrypt(0xbb5d12ba422834b5)
    chars="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!#$%&()=~|-^\`{@[+*};:],./_<>?\"\'"
    chars="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!#$%&()=~|-^\`{@[+*};:],./_<>?\"\'"
    for s1 in chars:
        for s2 in chars:
            for s3 in chars:
                #print(hex(int("534543434f4e7b"+("{0:x}".format(ord("a"),hex)),16)))
                #print((hex(int("534543434f4e7b"+("{0:x}".format(ord(s1),hex))+("{0:x}".format(ord(s2),hex))+("{0:x}".format(ord(s3),hex) ),16)), key_size=96, block_size=64)))
                #w = SimonCipher(hex(int("534543434f4e7b"+("{0:x}".format(ord(s1),hex))+("{0:x}".format(ord(s2),hex))+("{0:x}".format(ord(s3),hex) ),16)), key_size=96, block_size=64)
                #w = SimonCipher(hex(int("534543434f4e7b"+("{0:x}".format(ord(s1),hex)),16)), key_size=96, block_size=64)
                
                
                w = SimonCipher(int("534543434f4e7b"+("{0:x}".format(ord(s1)))+("{0:x}".format(ord(s2)))+("{0:x}".format(ord(s3))),16) , key_size=96, block_size=64)
                #print(w.encrypt(0x6d564d37426e6e71)==8502537985492285823)
                #exit()
                if w.encrypt(0x6d564d37426e6e71) == 13500967849109763253:
                    print("flag is "+s1+s2+s3)
                    exit()
